    ...
    <div className="space-y-6">
      <Section title="Comp Channels">
        <p className="text-sm text-text-secondary leading-relaxed">
          ...
          channels</span> (each its own root on disk, with its own staging/archive folders and its own
          edit-proposal and comp-file-proposal queues). Editor/contributor/manager access is a{' '}
          <span className="font-semibold text-text-primary">per-channel membership</span>, not just the old global
          profile flags (see Roles, Auth &amp; Accounts tab). This is a completely separate system from News
          channels (News tab) — different slugs, different roles, different storage;{' '}
          <Code>?channel=</Code> means something different depending which endpoint it&apos;s on.
        </p>
    ...
    <div className="space-y-6">
      <Section title="Roles">
        ...
        <p className="text-xs text-text-muted mt-2">
          ...
          proposals. <Code>manager</Code> is likewise independent — a flag that grants the proposal/comp-proposal
        ...
        <p className="text-xs text-text-muted mt-2">
          ...
          flag on. The global flag only ever covers the <span className="font-semibold text-text-primary">primary</span>{' '}
          channel — on any other channel, treat a missing membership row as "check the channel-scoped flag, not the
          global one." This is a completely separate system from News channels below — different slugs, different
          roles, different storage.
        </p>
...

{
  ...
  "news_subscriptions": ["announcements"],   // news-channel slugs the user follows
  "memberships": [
...

{
  ...
  "sort_order": 0
        ...
        <p className="text-xs text-text-muted">
          <Code>slug</Code> is generated from <Code>name</Code> when omitted. A new channel is never primary.
        </p>
...

{
  ...
  "auto_approve_comp_proposals": false
        ...
        <p className="text-xs text-text-muted">
          Note the field names here — <Code>editor_enabled</Code>/<Code>contributor_enabled</Code>/
          <Code>manager_enabled</Code> — differ from the <Code>is_editor</Code>/<Code>is_contributor</Code>/
          <Code>is_manager</Code> booleans the same membership shows up as inside <Code>account.memberships</Code>{' '}
          (Roles, above). Same flags, different name on write vs. read.
        </p>
        <p className="text-xs text-text-muted">
...
{
  "results": [ /* post objects — see shape below */ ],
  "count": 42,
...
FormData:
  ...
  "size": 1048576
        ...
        <p className="text-xs text-text-muted">
          The client classifies an attachment as image/audio by <Code>mime</Code> first (<Code>image/*</Code> /{' '}
          <Code>audio/*</Code>), falling back to a guess from <Code>name</Code>&apos;s extension when{' '}
          <Code>mime</Code> is missing or generic (e.g. <Code>application/octet-stream</Code>) — send an accurate{' '}
          <Code>mime</Code> where possible so that guess is never needed.
        </p>
        <p className="text-xs text-text-muted">
        ...
        <p className="text-xs text-text-muted">
          Client cap on upload size is 25MB, matching the server limit — files over that are rejected client-side
